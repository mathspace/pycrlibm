...
pycrlibm is a CPython extension binding CRlibm scalar mathematical functions; the repository vendors CRlibm, but source builds may resolve a host installation. Each wrapper converts one Python argument to a C `double`, calls a fixed native function and returns a Python floating-point value (`ext/crlibmmodule.c:9`). The README describes correctly rounded elementary functions; that numerical guarantee is an upstream claim, not independent verification of every compiler/platform combination (`README.rst:22`). The library does not parse an expression language or choose a native function pointer from input text.
...
| Extension builder | Resolve CRlibm through configured plus toolchain search paths; invoke make on a caught initial build exception | `setup.py:101`, `setup.py:153` |
...
| POSIX source build | Initial extension build using configured and normal compiler/linker search paths; any caught Exception → `make crlibm-notest` → retry | Added `<build-cwd>/build/crlibm/include` and `lib` paths, linked by name `crlibm`; an available system header/library can satisfy the first attempt without vendored make | Build tools and extension; workspace/toolchain and selected header/library provenance must be trusted | `setup.py:104`; `setup.py:157`; `Makefile:25` |
| Windows source build | MSYS2 compiler → Python DLL lookup; caught initial extension-build exception → `make msys2` | Same absolute CRlibm prefix, plus `build/crlibm/lib/&lt;Python-DLL-basename&gt;.a` | `gendef`, `dlltool`, compiler; OS/toolchain discovery controls | `setup.py:72`, `Makefile:14` |
...
| PEP 517 source build | Frontend resolves `setuptools<75.0.0` and loads `setuptools.build_meta:__legacy__`; acquisition depends on cache/index and isolation configuration | Selected backend package executes in the build environment before/with setup.py and influences produced native artifacts | Build frontend, package source/cache and backend code have build-time authority; no exact backend version/hash is declared here | `pyproject.toml:1`; `pyproject.toml:2`; `pyproject.toml:3` |

Build paths use the current build workspace. Any Exception caught from the initial extension build triggers native make and a second attempt, including compile failures before linking. If the initial build finds compatible headers/libraries through host toolchain search paths and succeeds, the vendored fallback does not run. The fallback target omits tests, whereas explicit `make crlibm` runs checks before installation (`setup.py:104`; `setup.py:157`; `Makefile:3`; `Makefile:6`).
...
Build inputs carry substantially greater authority than scalar values. `setup.py` executes Python build logic, compilers process local native sources, and the MSYS2 path discovers a Python DLL before generating an import library. Windows lookup includes a 32-bit `syswow64` substitution; the Makefile enables SSE2 and selects BSD-style CRlibm configuration (`setup.py:72`, `Makefile:16`). Caller or CI ownership of build directories and executable resolution therefore matters, as do system header/library selection and PEP 517 backend acquisition. The declared backend requirement is a version range, not an exact artifact binding; configured package sources and caches influence which backend executes (`setup.py:157`; `pyproject.toml:2`). A hostile installed extension is already native code in the process; that is not a new exploit caused by a numeric call.
...
| High, conditional | Lower-trust source, toolchain, system CRlibm or build backend becomes code executed during build or shipped/referenced by consumers | Attacker can influence a selected build input, package source/cache or native library below maintainer authority | Build-context compromise or modified native execution in consumers | Local include/library paths are additive; first successful build bypasses vendored make. PEP 517 declares a backend version range, not a verified package digest | Control backend sources/caches and bind backend/native dependencies to reviewed artifacts; verify actual header/library and static/dynamic dependency provenance | `setup.py:104`; `setup.py:153`; `Makefile:25`; `pyproject.toml:2` |
